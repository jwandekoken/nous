...
def mark_request_start() -> float:
    ...
    request_started_at_var.set(started_at)
    return started_at
...
def mark_request_end(started_at: float | None) -> float | None:
    ...
    request_duration_ms_var.set(duration_ms)
    return duration_ms
...
def clear_usage_context() -> None:
    ...
    request_started_at_var.set(None)
    request_duration_ms_var.set(None)
    request_method_var.set(None)
    request_path_var.set(None)
    tenant_id_var.set(None)
    graph_name_var.set(None)
    actor_type_var.set("unknown")
